<template>
  <div>
  <b-navbar toggleable="lg" type="dark"  fixed="top">
    <b-navbar-brand href="#">Rent Your Tools</b-navbar-brand>

    <b-navbar-toggle target="nav-collapse"></b-navbar-toggle>

    <b-collapse id="nav-collapse" is-nav>
      <b-navbar-nav>
        <b-nav-item to="/">Home</b-nav-item>
        <b-nav-item href="#" disabled>About</b-nav-item>
        <b-nav-item href="#" disabled>About</b-nav-item>
      </b-navbar-nav>

      <!-- Right aligned nav items -->
      <b-navbar-nav class="ml-auto">
        <b-nav-form>
          <b-form-input size="sm" class="mr-sm-2" placeholder="Search"></b-form-input>
          <b-button size="sm" class="my-2 my-sm-0" variant="warning">Search</b-button>
        </b-nav-form>

        <b-navbar-nav right>
          
        <b-nav-item to="/my-items">My items</b-nav-item>
        <b-nav-item to="/products">Products</b-nav-item>
      </b-navbar-nav>

        <!-- <b-nav-item-dropdown text="Lang" right>
          <b-dropdown-item href="#">EN</b-dropdown-item>
          <b-dropdown-item href="#">ES</b-dropdown-item>
        </b-nav-item-dropdown> -->

        <!-- <b-nav-item-dropdown right>
           //Using 'button-content' slot 
           <template #button-content>
            <em>User</em>
          </template>
          <b-dropdown-item href="#">Login</b-dropdown-item>
          <b-dropdown-item href="#">Register</b-dropdown-item>
        </b-nav-item-dropdown> -->
      </b-navbar-nav>
    </b-collapse>
  </b-navbar>
</div>
</template>

<script>
export default {

}
</script>

<style lang="scss" scoped>
nav{
    background: #590004;
}
</style>